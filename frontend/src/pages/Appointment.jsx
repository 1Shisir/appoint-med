...
const Appointment = () => {
  ...
  const getAvailableSlots = async () => {
    ...
    let timeSlots = []  //array to store time slots
      ...
      <div className='sm:ml-72 sm:pl-4 mt-4 font-medium text-gray-700'>
        ...
        <div className='flex items-center w-full gap-3 overflow-x-scroll mt-4'>
          {docSlots.length && docSlots[slotIndex].map( (item, index) => (
            <p onClick={() => setSlotTime(item.time)} className={`text-sm font-light flex-shrink-0 px-5 py-2 rounded-full cursor-pointer ${item.time === slotTime ? 'bg-primary text-white' : 'text-gray-500 border'}`} key = {index}>
