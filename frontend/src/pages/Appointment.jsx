...
const Appointment = () => {
  ...
  const getAvailableSlots = async () => {
    if (!docInfo || !docInfo.available) return; // Skip fetching slots if the doctor is unavailable
        ...
        {docInfo.available && (
          <div className="sm:ml-72 sm:pl-4 mt-4 font-medium text-gray-700">
                ...
                docSlots[slotIndex].map((item, index) => (
                  <p
                    ...
                    className={`text-sm font-light flex-shrink-0 px-5 py-2 rounded-full cursor-pointer ${
                      item.time === slotTime ? 'bg-primary text-white' : 'text-gray-500 border'
                    }`}
                    key={index}
                  >
            ...
            <button
              ...
              className="bg-primary text-white text-sm py-3 px-14 my-6 rounded-full font-light"
            >
              Book an appointment
            </button>
          </div>
